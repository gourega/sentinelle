...
const App: React.FC = () => {
  ...
  const [activeCitizens, setActiveCitizens] = useState(142);
  const [user, setUser] = useState<{ uid: string, displayName: string } | null>(null);
  const [isSyncing, setIsSyncing] = useState(false);

  // 1. Gestion de la Session Locale (Simulateur d'Auth)
  useEffect(() => {
  ...
  const handleLogin = async () => {
    ...
    showToast(`Connexion établie !`, 'info');
  };
